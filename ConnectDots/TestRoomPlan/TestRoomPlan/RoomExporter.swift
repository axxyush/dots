...
enum RoomExporter {
    ...
    private static func nearestWallIndex(for surface: CapturedRoom.Surface, in walls: [CapturedRoom.Surface]) -> Int? {
        ...
        let pos = simd_float3(surface.transform.columns.3.x, surface.transform.columns.3.y, surface.transform.columns.3.z)
        var bestIndex = 0
        var bestDist: Float = .infinity
        for (i, wall) in walls.enumerated() {
            let wp = simd_float3(wall.transform.columns.3.x, wall.transform.columns.3.y, wall.transform.columns.3.z)
    ...
    static func computeRoomDimensions(walls: [CapturedRoom.Surface]) -> (RoomBoundingBox, Double, Double) {
        ...
        for wall in walls {
            ...
            let dir = simd_normalize(simd_float3(wall.transform.columns.0.x, wall.transform.columns.0.y, wall.transform.columns.0.z))
            let a = simd_float3(c.x, c.y, c.z) + dir * hw
            let b = simd_float3(c.x, c.y, c.z) - dir * hw
            mnX = min(mnX, min(a.x, b.x)); mxX = max(mxX, max(a.x, b.x))
            mnY = min(mnY, min(a.y, b.y)); mxY = max(mxY, max(a.y, b.y))
            mnZ = min(mnZ, min(a.z, b.z)); mxZ = max(mxZ, max(a.z, b.z))
        }
        let bbox = RoomBoundingBox(
            minX: Double(mnX), maxX: Double(mxX),
            minY: Double(mnY), maxY: Double(mxY),
            minZ: Double(mnZ), maxZ: Double(mxZ)
        )
        return (bbox, Double(mxX - mnX), Double(mxZ - mnZ))
    }
