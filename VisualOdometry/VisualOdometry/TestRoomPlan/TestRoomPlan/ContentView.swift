...
struct ContentView: View {
    ...
    @State private var showNavigationBootstrap = false
    ...
    var body: some View {
        ...
        } else if showNavigationBootstrap {
            NavigationBootstrapView {
                showNavigationBootstrap = false
            }
            ...
            VStack {
                ...
                if introPhase == .actionsVisible {
                    VStack(spacing: 14) {
                        ...
                        Button {
                            Task { await startNavigationFlow() }
                        } label: {
                            HStack {
    ...
    @MainActor
    private func startNavigationFlow() async {
        let granted = await ensureCameraAccess()
        guard granted else {
            scanIssue = .cameraDenied
            return
